	...
	<?= GridView::widget([
		...
		'columns' => [      
			...
			[
				...
				'buttons' => [                                         
					'update' => function ($url, $model) {
						$url = Url::to(['/pipe/cutting/create', 'EditId' => $model->id]);
						if(!empty($_GET)){
                            $urlParams = Yii::$app->general->getFilters();
                            $urlParams[] = '/pipe/cutting/create';
			...
			[
				...
				'label' => Yii::$app->trans->getTrans('Wall Thickness'),
				'filter' => false,
				'value' => function($model){
					$pipeDetails = \app\models\Pipe::find()->select('wall_thikness')->where(['LIKE', 'pipe_number', $model->pipe_number])->active()->asArray()->one();
                    return !empty($pipeDetails) ? $pipeDetails['wall_thikness'] : '-';
				}
			],
			'length_1',
			'length_2',
