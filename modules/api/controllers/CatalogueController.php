...
use app\models\TaxonomyBinding;

class CatalogueController extends ActiveController
{
	
	public $modelClass = 'app\models\TaxonomyValue';
	...
	public function actionForChildList($child_taxonomy_id){
		$TaxonomyValueList = TaxonomyValue::find()->where(['taxonomy_id'=>$child_taxonomy_id,'project_id'=>Yii::$app->user->identity->project_id])->active()->asArray()->all();
		$Taxarray = [];
	...
	private function valusOfTaxonomy($taxonomy_id){
		$TaxonomyValueList = TaxonomyValue::find()->where(['taxonomy_id'=>$taxonomy_id,'project_id'=>Yii::$app->user->identity->project_id])->active()->asArray()->all();
		$Taxarray = [];
		if(!empty($TaxonomyValueList)){
			foreach($TaxonomyValueList as $ele){
