...
/**
 * CleangaugeSearch represents the model behind the search form of `app\models\Cleangauge`.
 */
class CleangaugeSearch extends Cleangauge
{
    /**
     * @inheritdoc
     */
    ...
    public function search($params)
    {
        ...
        if(Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly"){
            $query = Cleangauge::find()->anomally()->orderBy('id DESC');;
        }else{
            $query = Cleangauge::find()->active()->orderBy('id DESC');;
        }
