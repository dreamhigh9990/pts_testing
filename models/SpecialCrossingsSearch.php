...
/**
 * SpecialCrossingsSearch represents the model behind the search form of `app\models\SpecialCrossings`.
 */
class SpecialCrossingsSearch extends SpecialCrossings
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        return [
            [['id', 'from_kp', 'to_kp', 'project_id', 'qa_manager', 'created_by', 'updated_by', 'created_at', 'updated_at', 'is_deleted', 'is_active'], 'integer'],
    ...
    public function search($params)
    {
        if(Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly"){
            $query = SpecialCrossings::find()->anomally()->orderBy('id DESC');;
        }else{
            $query = SpecialCrossings::find()->active()->orderBy('id DESC');;
        }
