...
/**
 * LoweringSearch represents the model behind the search form of `app\models\Lowering`.
 */
class LoweringSearch extends Lowering
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            ...
            [['date', 'report_number', 'holiday_detection', 'holiday_serial_number', 'check_points', 'comment', 'signed_off', 'is_anomally', 'why_anomally'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        if(Yii::$app->controller->module->id == "admin" && Yii::$app->controller->id == "anomaly"){
            $query = Lowering::find()->anomally()->orderBy('from_kp ASC');;
        }else{
            $query = Lowering::find()->active()->orderBy('from_kp ASC');
        }
        ...
        $query->andFilterWhere(['like', 'report_number', $this->report_number])
            ->andFilterWhere(['like', 'holiday_detection', $this->holiday_detection])
            ->andFilterWhere(['like', 'holiday_serial_number', $this->holiday_serial_number])
